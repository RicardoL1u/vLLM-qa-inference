def load_prompt(model_name,shot_num=1):
    ...
    print(model_name)
    print("---")
    print('CodeLlama' in model_name)
    print('Instruct' in model_name)
    print('---')
    ...
    elif 'llama-2' in model_name and 'chat' in model_name and 'wo-psg' in model_name:
        return open(f'{prompt_folder}/llama-2-chat-wo-psg.txt').read()
    elif 'llama-2' in model_name and 'chat' in model_name:
        return open(f'{prompt_folder}/llama-2-chat.txt').read()
    elif 'codellama' in model_name and 'instruct' in model_name:
        return open(f'{prompt_folder}/llama-2-chat.txt').read()
    elif 'instr+opin' in model_name:
        return open(f'{prompt_folder}/instr+opin.txt').read()
    elif 'instr+attr' in model_name:
        return open(f'{prompt_folder}/instr+attr.txt').read()
    elif 'wo-psg' in model_name:
        return open(f'{prompt_folder}/foundation_model-wo-psg.txt').read()
    else:
